...
public class Assignment5Part1 extends TextProgram {
    ...
    private static String GOL = "aeiouy";
    ...
    private int syllablesIn(String word) {
        ...
        if (cntr > 1 && !isCharVowel(word.charAt(word.length() - 2)) && word.charAt(word.length() - 1) == 'e') cntr--;
    ...
    private boolean isCharVowel(char ch) {
        for (int i = 0; i < GOL.length(); i++) {
            //if(GOL.charAt(i) == ch) return true;
            if (((Character) GOL.charAt(i)).toString().equalsIgnoreCase(((Character) ch).toString())) return true;
        }
        return false;
    }
